...
# PoseResult moved to the torch-free src.core.models so consumers that only need the
# dataclass (the geometric feature extractor, the search engine's flip-search re-extraction)
# don't drag torch/mmpose into faiss-only processes — e.g. the search server, where a second
# OpenMP runtime aborts the process (OMP Error #15). Re-exported here so existing
# `from src.core.pose_detector import PoseResult` keeps working unchanged.
