...
- **`scripts/`** — operational utilities (migration runners, `diagnose_multiperson.py`, `prepare_eject.py`, `kill_db_connections.sh`), not part of the app runtime. Two matter when touching features: `reextract_features.py` (re-extracts all stored geometric features + rebuilds FAISS — **required after any change to GeometricFeatureExtractor's vector layout or normalization**, else stored vectors and fresh queries become incomparable; run per `DB_PROFILE`) and `benchmark_occlusion.py` (synthetic-occlusion self-similarity benchmark for the detection→features→masking stack).
